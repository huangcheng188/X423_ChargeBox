...
#define YHM4001_MASTER 0x64<<1   //8bit address
 ...
 static int i2c_write (uint8_t i2c_addr, uint8_t register_addr, char* buffer, uint8_t Nbyte )
{
    ...
    // add huangcheng 2005-04-15
    HAL_StatusTypeDef ret;
    uint8_t *ptr;

    ret = HAL_I2C_Mem_Write(&hi2c2, i2c_addr, register_addr, I2C_MEMADD_SIZE_8BIT, buffer, 1, HAL_MAX_DELAY);
    printf("WR:ret=%d register_addr=0x%02x buffer=0x%02x\r\n", ret, register_addr, buffer[0]);
...
static int i2c_read (uint8_t i2c_addr, uint8_t register_addr, char* buffer, uint8_t Nbyte )
{
    ...
    ret = HAL_I2C_Mem_Read(&hi2c2, i2c_addr, register_addr, I2C_MEMADD_SIZE_8BIT, buffer, Nbyte, HAL_MAX_DELAY);
    for(int i=0; i<Nbyte; i++){
      printf("RD:ret=%d buffer[i]=0x%02x \r\n", ret, i, buffer[i]);
    }
...
// {
//   printf("L 64        //-----list all of register value\r\n"); 
//   printf("S 64  0     //-----read reg0 value\r\n"); 
//   printf("W 64  0  cd //-----write reg 0==0xcd\r\n"); 

//   printf("D 91       //Plug IN detection_Left channel\r\n"); 
//   printf("D 92       //Plug OUT detection_Left channel\r\n"); 
//   printf("D 93       //Master_Intb_To_VoutL_Uart\r\n"); 
//   printf("D 94       //Master_Intb_To_VoutR_Uart\r\n"); 
//   printf("D 95       //Master_Exit_Uart\r\n");    
//   printf("D 96       //Master_Enter_Charge_Mode\r\n");            
// }
//*******************************************************************************************************************
void print_help_information(void)
{
    ...
    if((temp_rxdata[2]&0x40)==0x40)
     {
         ...
         printf("Master: L is  plug out status, now do the plug in test \r\n");   
     }
    else
    {
        temp_txdata[0]=0x91;                                                           
        i2c_write(YHM4001_MASTER, 0x13, &temp_txdata[0], 1);   // master switch L ON
         ...
         L_status=0;  //0== in box
         printf("********Master: L is  plug in, so exit the plug in test program*******\r\n");   
...
if(L_status==1)
{
    ...
    {
       printf("\r\n");   
       printf("Master: L plug in\r\n");   
       printf("***************Master: L plug in detect finished****************\r\n");  
...
//-------------now, wait the interrupt pin is low----------------
    ...
    {
       printf("\r\n");   
       printf("Master: L  Current Min\r\n");   
       printf("Now do the 2uA Isource Detect\r\n");
    ...
    i2c_write(YHM4001_MASTER, 0x0d, &temp_txdata[0], 1);   ///mask all of interrupt  
       ...
       {
         printf("Master: L No Plug out, is a current dither\r\n"); 
         printf("Master: need Re-do the plug out detect\r\n"); 
  ...
  printf("****************Master: Plug Out Detect finished************************\r\n"); 
